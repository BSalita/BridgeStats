...
dds = cdll.LoadLibrary(r"c:\sw\bridge\dds\dds-291\vs2022\x64\Release\dds.dll") # patch for windows
#dds = cdll.LoadLibrary(r"C:/sw\bridge\dds\dds-291\src\x64\Release\dds.dll") # patch for windows
# C:\sw\bridge\dds\dds-291\src\x64\Release\dds-291.dll
print('Loaded lib {0}'.format(dds))

DDS_VERSION = 20900 # patch - 6-Aug-2021 - was 20700 
...
MAXNOOFTABLES = 40 # patch - 6-Aug-2021 - added - MAXNOOFBOARDS erroneously used
...
class ddTableDeals(Structure):
    _fields_ = [("noOfTables", c_int),
                ("deals", ddTableDeal * MAXNOOFTABLES)]
...
class ddTableDealsPBN(Structure):
    _fields_ = [("noOfTables", c_int),
                ("deals", ddTableDealPBN * MAXNOOFTABLES)]

class ddTableResults(Structure):
#    _fields_ = [("resTable", c_int * DDS_STRAINS * DDS_HANDS)] # well, well, so this goofs up whereas next line is ok!
    _fields_ = [("resTable", c_int * DDS_HANDS * DDS_STRAINS)] # how is it possible that ordering of HANDS/STAINS makes a diff?

class ddTablesRes(Structure):
    _fields_ = [("noOfBoards", c_int),
                ("results", ddTableResults * MAXNOOFTABLES)]
...
class parResultsMasters(Structure): # patch - 6-Aug-2021 - added
    _fields_ = [("presults", parResultsMaster * MAXNOOFTABLES)]
